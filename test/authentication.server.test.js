...
const ts = Date.now();
const adminTs = `auth.test${ts}`;
const appTs = `app.user${ts}`;

describe('POST /auth/register for Admin User', () => {
  it('should register Admin user and return JWT token', (done) => {
    chai.request(server)
      .post('/auth/register')
      .send({
        first_name: 'Art',
...
describe('POST /auth/register for App User', () => {
  it('should register app user and return JWT token', (done) => {
    chai.request(server)
      .post('/auth/register')
      .send({
        first_name: 'App',
...
describe('Login Admin user', () => {
  it('should login and retrieve JWT token for Admin user', (done) => {
    chai.request(server)
      .post('/auth/login')
      .send({
  ...
  it(`should read admin user ${adminUserId}`, (done) => {
    chai.request(server)
      .get(`/api/v2/users/${adminUserId}`)
  ...
  it(`should read app user ${appUserId}`, (done) => {
    chai.request(server)
      .get(`/api/v2/users/${appUserId}`)
      .set('Authorization', token)
      .end((err, res) => {
        res.should.have.status(200);
        res.should.be.json;
...
// Delete App user
describe('Delete App user', () => {
  it('should delete app user', (done) => {
    chai.request(server)
      .delete(`/api/v2/users/${appUserId}`)
      .set('Authorization', token)
      .end((err, res) => {
        res.should.have.status(200);
        res.should.be.json;
...
describe('Delete Admin user', () => {
  it('should delete Admin user', (done) => {
    chai.request(server)
      .delete(`/api/v2/users/${adminUserId}`)
      .set('Authorization', token)
      .end((err, res) => {
        res.should.have.status(200);
        res.should.be.json;
